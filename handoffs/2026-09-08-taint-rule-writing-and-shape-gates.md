...
Owner: claude
Branch: `fix/adding-a-rule-must-not-rewrite-the-file` (PR #56), then
`feat/sink-rules-name-methods-not-receivers` (PR #57, stacked on it)
Date: 2026-09-08
...
- Argument-shape gates fail open by design: an argument whose shape cannot be
  ...
  class is unmeasured.
- PR #57 has only 2 checks because its base is PR #56's branch; most workflows
  run on pull requests into `main` only. It gets full CI when it retargets.

## Follow-ups

- Merge #56, let #57 retarget `main`, and confirm #57's full check set.
