...
def _load_json_object_arg(value: str | None, arg_name: str) -> dict | None:
    """Parse a JSON object from an inline value or readable file path."""
    if not value:
        return None
...
def main():
    ...
    task_parser.add_argument("--intent", choices=["refactor", "bugfix", "feature", "cleanup", "migration"], default="refactor", help="Plan intent")
    task_parser.add_argument("--project", help="Project name for scoped analysis")
    task_parser.add_argument("--task-contract", help="Gate contract JSON object or path to a JSON file")
    task_parser.add_argument("--current-state", help="Gate current-state JSON object or path to a JSON file")
...
def cmd_task(args):
    ...
    if args.action == "validate":
        if result.get("lint_passed") is False or result.get("tests_passed") is False:
            should_fail = True
    ...
    commands = [
        ...
        },
        {
            ...
            "args": [
                ...
                {"name": "--project", "type": "string", "required": False, "description": "Project name for scoped analysis"},
                {"name": "--task-contract", "type": "json|string", "required": False, "description": "Gate contract JSON object or JSON file path"},
                {"name": "--current-state", "type": "json|string", "required": False, "description": "Gate state JSON object or JSON file path"},
