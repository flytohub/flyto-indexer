    ...
    if result.get("task_profile", {}).get("compound"):
        ...
        if parts:
            return (
                ...
                "2. At ⛔ GATE steps, call task_gate_check. If pass=false, execute every "
                "required_actions item, update current_state with the exact requested keys, "
                "and immediately re-run the same gate until pass=true.\n"
                "3. Do NOT edit code or enter the blocked phase while a gate is false.\n"
                "4. Ask the user only when remediation needs unavailable authorization, "
                "required input, or an external state change."
            )
        ...
        if plan:
            ...
            return (
                ...
                "required_actions item, update current_state with the exact requested keys, "
                "and immediately re-run the same gate until pass=true.\n"
                "3. Do NOT read/edit source files or enter the blocked phase while a gate is false.\n"
                "4. Ask the user only when remediation needs unavailable authorization, "
                "required input, or an external state change.\n"
                "5. After completing all steps, proceed with changes."
            )
