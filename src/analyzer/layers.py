...
# Structural parser for .vue source, never a sanitizer. The closing pattern
# accepts whitespace and stray attributes (`</script\t\n bar>`) so a block is
# not silently missed — CodeQL py/bad-tag-filter flags the narrow form.
_VUE_SCRIPT_BLOCK = re.compile(
    r"<script\b[^>]*>(.*?)</\s*script\b[^>]*>",
