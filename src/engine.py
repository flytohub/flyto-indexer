...
from .tree_sitter_adapter import OptionalTreeSitterAdapter
...
def _index_pipeline_fingerprint(parser_signature: str = "") -> str:
    """Hash scanner implementations so parser changes invalidate cached files."""
    digest = hashlib.sha256()
    digest.update(b"flyto-index-pipeline.v2\0")
    digest.update(f"python-{sys.version_info.major}.{sys.version_info.minor}".encode())
    digest.update(b"\0")
    digest.update(parser_signature.encode("ascii", errors="ignore"))
    scanner_root = Path(__file__).resolve().parent / "scanner"
    try:
        scanner_files = sorted(scanner_root.glob("*.py"))
    except OSError:
        scanner_files = []
...
class IndexEngine(
    ...
    def __init__(
        ...
        self.tree_sitter = OptionalTreeSitterAdapter()
        self.pipeline_fingerprint = _index_pipeline_fingerprint(
            self.tree_sitter.signature()
        )
    ...
    def scan(self, incremental: bool = True) -> dict:
        ...
        for rel_path in files_to_scan:
            ...
            try:
                ...
                self.tree_sitter.inspect(Path(rel_path), content, symbols)
        ...
        return {
            ...
            "pipeline_fingerprint": self.pipeline_fingerprint,
            "parser": self.tree_sitter.summary(),
